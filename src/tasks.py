...
async def stat_task(stat_bot: Bot, session_factory: async_sessionmaker[AsyncSession]) -> None:
    while True:
        ...
        async with session.begin():
            ...
            all_logs = await system_log_repository.get_logs_order_by_time_and_limit(system.last_user_log, system.max_user_cumulative)
        ...
        if all_logs:
            text = "\n\n".join([f"{i+1}. {log.created_at} command: {log.command} \n {log.user.info}" for i, log in enumerate(all_logs)])
            try:
